...
const CandidatesPage = () => {
    ...
    // שימוש ב-useEffect כדי לטעון את ההערות
    useEffect(() => {
        fetchNotes();
    }, []);
    // הוספת הערה חדשה
    const addNote = async () => {
        ...
        try {
            const newNote: Partial<Note> = {
                ...
                userId: selectedCandidate.id, // אם יש שדה כזה
                content: newNoteText, // שדה חדש
    ...
    const candidateNotes = notes.filter(
        (note) => note.userId === selectedCandidate?.id,
    )
            ...
            <NotesDrawer anchor="right" open={notesDrawerOpen} onClose={() => setNotesDrawerOpen(false)}>
                ...
                <Box>
                    ...
                    <Button
                        ...
                        color="primary"
                        onClick={addNote}
                        disabled={!newNoteText.trim() || !selectedCandidate} // Disable if no text or no selected candidate
                    >
                        הוסף הערה
                    </Button>
                </Box>
